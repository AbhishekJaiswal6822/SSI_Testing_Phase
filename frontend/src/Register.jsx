...
//     // Updated setMemberCount to handle input field updates and limits (FIX FOR INPUT BUG)
//     const setMemberCount = (countStr) => {
//         // FIX: Handle empty string ("") by parsing, defaulting to 1 (min member count) if invalid input.
//         const parsedCount = parseInt(countStr) || 1; 
        
//         // n is the actual number of members we will set in state (capped at MAX_GROUP_MEMBERS)
//         const n = Math.max(1, Math.min(MAX_GROUP_MEMBERS, parsedCount)); 
        
//         // If the user tries to enter a number > MAX_GROUP_MEMBERS, show the toast
//         // We only show the toast if the user manually types a number greater than the max limit
...
//                 'state', 'emergencyName', 'emergencyPhone', 'tshirtSize', 'cause', 
...
//                                     <select value={charityParticipant.tshirtSize} onChange={(e) => handleCharityParticipantChange('tshirtSize', e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm bg-white shadow-sm focus:border-teal-500 focus:ring-2 focus:ring-teal-500/50" required id="charity-tshirtSize">
