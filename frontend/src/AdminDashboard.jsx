...
//     // Search logic covers individual names and group names
//     const firstName = r.runnerDetails?.firstName || r.groupMembers?.[0]?.firstName || "";
//     const lastName = r.runnerDetails?.lastName || r.groupMembers?.[0]?.lastName || "";
//     const groupName = r.groupName || "";
...
//   // Flat mapping for the table so each Group Member gets their own row
//   const tableRows = filteredRegistrations.flatMap((r) => {
...
//           <input
//             type="text"
//             placeholder={`Search by Name, Group, Email, or ID...`}
//             className="w-full pl-11 pr-12 py-3 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-teal-500/10 transition bg-white text-sm"
//             value={searchTerm}
//             onChange={(e) => setSearchTerm(e.target.value)}
//           />
...
//         <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
//           {((activeTab === "users" && filteredUsers.length > 0) || (activeTab === "registrations" && tableRows.length > 0)) ? (
//             <div className="overflow-x-auto custom-scrollbar">
...
//                         <th className="p-5 sticky left-[192px] bg-slate-50 z-30 w-48 border-r">Last Name</th>
//                         <th className="p-5 w-48">Group Name / Master Account</th>
//                         <th className="p-5 w-40">Registration Type</th>
//                         <th className="p-5 w-48 text-teal-600">Registration Category</th>
//                         <th className="p-5 w-64">Email</th>
//                         <th className="p-5 w-48">Phone Number</th>
...
//                         <th className="p-5 w-48 text-slate-400">PG Fee</th>
//                         <th className="p-5 w-48 text-slate-400">GST Amount</th>
//                         <th className="p-5 w-48 font-black text-slate-900 bg-slate-50">Total Amount</th>
//                         <th className="p-5 w-40">ID Type</th>
//                         <th className="p-5 w-64">ID Number</th>
//                         <th className="p-5 w-64">ID Path</th>
//                         <th className="p-5 w-64">Order ID</th>
...
//                   ) : (
//                     tableRows.map((r) => (
//                       <tr key={r.rowId} className="hover:bg-slate-50 transition text-[12px] font-bold text-slate-900 uppercase">
//                         <td className="p-5 sticky left-0 bg-white z-10 border-r" style={{ width: '192px', minWidth: '192px' }}>{r.displayDetails?.firstName || "---"}</td>
//                         <td className="p-5 sticky left-[192px] bg-white z-10 border-r" style={{ width: '192px', minWidth: '192px' }}>{r.displayDetails?.lastName || "---"}</td>
                        
//                         <td className="p-5 text-teal-700 font-black">{r.groupName || "Individual Account"}</td>
//                         <td className="p-5 text-slate-400 font-black">{r.registrationType}</td>
//                         <td className="p-5 text-teal-600 font-black">{r.raceCategory}</td>
//                         <td className="p-5 lowercase font-medium">{r.displayDetails?.email || "---"}</td>
//                         <td className="p-5 font-mono">{r.displayDetails?.phone || "---"}</td>
//                         <td className="p-5 font-mono">{r.displayDetails?.whatsapp || "---"}</td>
//                         <td className="p-5">{formatDate(r.displayDetails?.dob)}</td>
//                         <td className="p-5">{r.displayDetails?.gender || "---"}</td>
//                         <td className="p-5 text-rose-600">{r.displayDetails?.bloodGroup || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.nationality || "---"}</td>
//                         <td className="p-5 whitespace-normal break-words" style={{ width: '500px', minWidth: '500px' }}>{r.displayDetails?.address || r.runnerDetails?.address || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.city || r.runnerDetails?.city || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.state || r.runnerDetails?.state || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.pincode || r.runnerDetails?.pincode || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.country || r.runnerDetails?.country || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.experience || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.finishTime || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.dietary || "---"}</td>
//                         <td className="p-5 font-black">{r.displayDetails?.tshirtSize || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.parentName || "---"}</td>
//                         <td className="p-5">{r.displayDetails?.parentPhone || "---"}</td>
                        
//                         <td className="p-5">₹{r.registrationFee || r.displayDetails?.registrationFee || 0}</td>
//                         <td className="p-5 italic text-slate-400">{r.couponCode || "NONE"}</td>
...
//                         <td className="p-5 font-black bg-slate-50 border-x">₹{r.amount}</td>
...
//                         </td>
//                         <td className="p-5 font-mono text-[10px] text-slate-400">{r.paymentDetails?.orderId || "---"}</td>
//                         <td className="p-5 font-mono text-teal-700">{r.paymentDetails?.paymentId || "---"}</td>
//                         <td className="p-5">
//                           <span className={`px-3 py-1 rounded-full text-[10px] font-black text-white ${r.paymentStatus === 'paid' ? 'bg-teal-600' : 'bg-amber-500'}`}>
//                             {r.paymentStatus}
//                           </span>
...
//         displayDetails: member,
//         isGroupMember: true,
//         memberPosition: `Member ${index + 1}`,
//         rowId: `${r._id}-${index}`
//       }));
//     }
//     // Individual & Charity mapping
//     return [{
//       ...r,
//       displayDetails: r.runnerDetails,
//       isGroupMember: false,
//       memberPosition: "Individual",
//       rowId: r._id
//     }];
//   });
...
//         {activeTab === "registrations" && (
//           <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 mb-10 text-slate-900">
//             {Object.entries(statsToDisplay).map(([key, val]) => (
...
//           <input
//             type="text"
//             placeholder={`Search by Name, Group Name, Email, or ID...`}
//             className="w-full pl-11 pr-12 py-3 rounded-2xl border border-slate-200 focus:ring-4 focus:ring-teal-500/10 transition bg-white text-sm"
//             value={searchTerm}
//             onChange={(e) => setSearchTerm(e.target.value)}
//           />
...
//                         <th className="p-5 sticky left-0 bg-slate-50 z-30 w-48 border-r">First Name</th>
//                         <th className="p-5 sticky left-[192px] bg-slate-50 z-30 w-48 border-r">Last Name</th>
//                         <th className="p-5 w-48 text-teal-700">Group/Pos</th>
//                         <th className="p-5 w-40">Registration Type</th>
//                         <th className="p-5 w-48 text-teal-600">Registration Category</th>
...
//                         <td className="p-5 text-slate-700 font-medium">{u.email}</td>
//                         <td className="p-5 text-slate-900 font-mono text-[12px] font-bold tracking-tight">{u._id}</td> 
//                         <td className="p-5 text-slate-900 font-bold font-mono">{u.phone || "N/A"}</td>
//                         <td className="p-5 text-slate-900 font-bold">{new Date(u.createdAt).toLocaleDateString()}</td>
//                       </tr>
//                     ))
//                   ) : (
...
//                         <td className="p-5 sticky left-[192px] bg-white z-10 border-r" style={{ width: '192px', minWidth: '192px' }}>{r.displayDetails?.lastName || "N/A"}</td>
                        
//                         <td className="p-5 text-teal-700 font-black">{r.groupName ? `${r.groupName} (${r.memberPosition})` : "Individual"}</td>
...
//                         {/* Address logic: checks member specific, then master, then N/A */}
//                         <td className="p-5 whitespace-normal break-words" style={{ width: '500px', minWidth: '500px' }}>
//                           {r.displayDetails?.address || r.runnerDetails?.address || "N/A"}
//                         </td>
...
//                         <td className="p-5 text-slate-500">₹{r.pgFee || 0}</td>
//                         <td className="p-5 text-slate-500">₹{r.gstAmount || 0}</td>
//                         <td className="p-5 font-black bg-slate-50 border-x">₹{r.amount || 0}</td>
...
//               {/* FILTER BY TYPE */}
//               <div className="flex flex-col gap-3">
//                 <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Filter By Type</span>
//                 <div className="flex flex-wrap gap-2">
//                   {['all', 'individual', 'group', 'charity'].map(t => (
//                     <button key={t} onClick={() => setRegFilter(t)} className={subTabClasses(regFilter === t)}>{t}</button>
//                   ))}
...
//               {/* PAYMENT STATUS */}
//               <div className="flex flex-col gap-3">
//                 <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Payment Status</span>
//                 <div className="flex flex-wrap gap-2">
//                   {['all', 'paid', 'pending', 'rejected'].map(s => (
//                     <button key={s} onClick={() => setStatusFilter(s)} className={subTabClasses(statusFilter === s)}>{s}</button>
//                   ))}
...
//               {/* RACE DISTANCE */}
//               <div className="flex flex-col gap-3">
//                 <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Race Distance</span>
//                 <div className="flex flex-wrap gap-2">
//                   {['all', '5k', '10k', '21k', '35k', '42k'].map(r => (
//                     <button key={r} onClick={() => setCatFilter(r)} className={subTabClasses(catFilter === r)}>{r}</button>
//                   ))}
...
//                 </thead>
//                 <tbody className="divide-y divide-slate-100">
//                   {(activeTab === "users" ? filteredUsers : tableRows).map((r) => (
...
//                           <td className="p-5 text-rose-600">{r.displayDetails?.bloodGroup || "N/A"}</td>
//                           <td className="p-5">{r.displayDetails?.nationality || "N/A"}</td>
//                           <td className="p-5 whitespace-normal break-words" style={{ width: '500px', minWidth: '500px' }}>{r.displayDetails?.address || r.runnerDetails?.address || "N/A"}</td>
//                           <td className="p-5">{r.displayDetails?.city || r.runnerDetails?.city || "N/A"}</td>
//                           <td className="p-5">{r.displayDetails?.state || r.runnerDetails?.state || "N/A"}</td>
//                           <td className="p-5">{r.displayDetails?.pincode || r.runnerDetails?.pincode || "N/A"}</td>
//                           <td className="p-5">{r.displayDetails?.country || r.runnerDetails?.country || "N/A"}</td>
...
//   // --- COMPREHENSIVE SEARCH FILTER FOR REGISTRATIONS ---
...
//     // Mapping every requested field to a searchable blob
//     const searchBlob = [
//       r.displayDetails?.firstName,
//       r.displayDetails?.lastName,
//       r.displayDetails?.email,
//       r.displayDetails?.phone,
//       r.displayDetails?.whatsapp,
//       r.displayDetails?.gender,
//       r.displayDetails?.bloodGroup,
//       r.displayDetails?.nationality,
//       r.displayDetails?.address,
//       r.displayDetails?.city,
//       r.displayDetails?.state,
//       r.displayDetails?.pincode,
//       r.displayDetails?.country,
//       r.displayDetails?.experience,
//       r.displayDetails?.finishTime,
//       r.displayDetails?.dietary,
//       r.displayDetails?.tshirtSize,
//       r.displayDetails?.parentName,
//       r.displayDetails?.parentPhone,
//       r.registrationType,
//       r.raceCategory,
//       r.groupName,
//       r.paymentStatus,
//       r.paymentDetails?.paymentId,
//       r.paymentDetails?.orderId,
//       r.idProof?.idNumber,
//       r.idProof?.idType,
//       r.amount?.toString(),
//       r.registrationFee?.toString(),
//       r.gstAmount?.toString(),
//       formatDate(r.displayDetails?.dob),
//       formatDate(r.paymentDetails?.paidAt)
//     ].join(" ").toLowerCase();
...
//   const statsToDisplay = {
//     matches: filteredRegistrations.length,
//     individual: regs.filter(r => r.registrationType === 'individual').length,
//     group: regs.filter(r => r.registrationType === 'group').length,
//     charity: regs.filter(r => r.registrationType === 'charity').length,
//     paid: regs.filter(r => r.paymentStatus === 'paid').length,
//     pending: regs.filter(r => r.paymentStatus === 'pending' || r.paymentStatus === 'Pending Payment').length,
//     rejected: regs.filter(r => r.paymentStatus === 'rejected').length,
//     revenue: regs.filter(r => r.paymentStatus === 'paid').reduce((acc, curr) => acc + (Number(curr.amount) || Number(curr.runnerDetails?.amount) || 0), 0)
//   };
...
//                       <tr key={u._id} className="hover:bg-slate-50 transition">
//                         <td className="p-5 font-bold text-teal-800 uppercase">{u.name}</td>
//                         <td className="p-5 text-slate-700 font-medium lowercase">{u.email}</td>
//                         <td className="p-5 text-slate-900 font-mono text-[12px] font-bold tracking-tight">{u._id}</td> 
//                         <td className="p-5 text-slate-900 font-bold font-mono">{u.phone || "N/A"}</td>
//                         <td className="p-5 text-slate-900 font-bold">{new Date(u.createdAt).toLocaleDateString()}</td>
//                       </tr>
...
function AdminDashboard() {
  ...
  // --- RE-CALCULATE STATS BASED ON FILTERED RESULTS ---
  const dynamicStats = {
    matches: filteredRegistrations.length,
    individual: filteredRegistrations.filter(r => r.registrationType === 'individual').length,
    ...
    revenue: filteredRegistrations.filter(r => r.paymentStatus === 'paid')
      .reduce((acc, curr) => {
        // Only count revenue once per unique registration ID to avoid double-counting group members
